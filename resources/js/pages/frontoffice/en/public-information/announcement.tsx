  ...
  return (
    <div className="flex flex-col">
            ...
            {topAnnouncements.map((item) => (
              <div key={item.id} className="overflow-hidden  border-0 px-6">
                ...
                <div className="p-4 text-white">
                  <Link href={`/en/news/pengumuman/${item.id}`} className="font-bold block hover:underline text-xl">
                    {item.title}
                  </Link>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {announcements.map((item) => (
              <div key={item.id} className="p-6 rounded-2xl">
                <Link
                  href={`/en/news/pengumuman/${item.id}`}
                  className="text-xl font-semibold text-[color:var(--primary-navy)] dark:text-white hover:underline"
                >
                  {item.title}
                </Link>
                <p className="text-base text-gray-600 mt-1 dark:text-white">{item.created_at} • Announcement</p>
      ...
      <div className="flex justify-start mb-10">
        <AppContainer>
          <Link href="/en/news/pengumuman" className="w-fit self-start">
