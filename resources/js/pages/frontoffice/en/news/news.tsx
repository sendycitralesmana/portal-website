      ...
      <ThemeProvider defaultTheme="dark" storageKey="vite-ui-theme">
        ...
        <main className="w-full">
          <div className="mt-6 md:mt-10 mb-6 md:mb-10 container">
            <Breadcrumb>
              <BreadcrumbList className="text-base md:text-xl text-[color:var(--primary-navy)] dark:text-white">
                ...
                <BreadcrumbSeparator />
                <BreadcrumbItem>
                  <BreadcrumbPage className="font-semibold text-[color:var(--primary-navy)] dark:text-white">
                    News
                  </BreadcrumbPage>
                </BreadcrumbItem>
                <BreadcrumbSeparator />
          ...
          <form onSubmit={onSubmit} className="container flex flex-row gap-2">
            <Input name="search" defaultValue={search} className="flex-1" placeholder={`Search ${slug}`} />
          ...
          <section className="container w-full">
            <section className="w-full grid grid-cols-1 lg:grid-cols-3 pt-5 gap-4">
              {data.data.length > 0 ? (
                data.data.map((item) => (
                  <Link key={item.id} href={`/en/news/${slug}/${item.id}`}>
                    <Card className="relative overflow-hidden aspect-[3/2] group bg-gray-100 shadow-lg rounded-lg hover:shadow-xl transition-shadow duration-300">
                      <div className="absolute inset-0 z-0">
                        <img
                          ...
                          loading="lazy"
                          onError={(e) => {
                            (e.currentTarget as HTMLImageElement).src = "/images/default.webp";
                          }}
                        />
                ...
                <div className="col-span-full flex flex-col items-center justify-center text-center 
                  ...
                  <p className="text-lg text-gray-500 dark:text-gray-400 mt-1">
                    Please check back later or try using a different keyword.
                  </p>
                </div>
              )}
            </section>
