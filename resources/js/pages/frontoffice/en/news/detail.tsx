      ...
      <main className="w-full">
        ...
        <AppContainer className="flex flex-col gap-4 pt-5">
          ...
          <div className="flex flex-col gap-1">
            ...
            {news.user && <small>author by: {news.user.name}</small>}
          </div>
          ...
          {news.documents.length > 0 && (
            <div className="flex flex-col gap-4 pt-5">
              <h3 className="font-semibold text-[color:var(--primary-navy)] dark:text-white">Related Documents</h3>
              <div className="grid grid-cols-3 gap-4">
                {news.documents.map((doc) => (
                  <div key={doc.id} className="flex items-center justify-between border p-3 rounded">
                    <span className="font-medium truncate">{doc.name}</span>
                    <Button size="icon" asChild>
                      <a href={`/en/news/document/${doc.id}`} target="_blank">
                        <DownloadIcon />
                      </a>
                    </Button>
                  </div>
          ...
          {news.images.length > 0 && (
            <div className="flex flex-col gap-4 pt-5">
              <h3 className="font-semibold text-[color:var(--primary-navy)] dark:text-white">Related Images</h3>
