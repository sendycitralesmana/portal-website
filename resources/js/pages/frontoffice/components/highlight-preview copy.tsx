...
export default function ImageSliderModal({ modals }: { modals: any[] }) {
                ...
                >
                    {modals.map((modal: any, i: number) => (
                        ...
                        >
                            ...
                            />
                            <div className="relative z-10 w-full h-full flex flex-col justify-end px-6 pb-24 bg-gradient-to-t from-black/70 via-black/10 to-transparent">
                                {/* <p className="text-2xl font-bold lg:text-4xl leading-snug break-words">
                                    {modal.news.title}
                                </p> */}
                                {/* <Link href={`/berita/${modal.news.news_category.slug}/${modal.news.id}`} className="text-2xl font-bold lg:text-4xl leading-snug break-words">
                                    {modal.news.title}
                                </Link> */}
                                {modal.news.title !== "-" && (
                                <Link
                                    href={`/berita/${modal.news.news_category.slug}/${modal.news.id}`}
                                    className="text-2xl font-bold lg:text-4xl leading-snug break-words"
                                >
                                    {modal.news.title}
                                </Link>
                                )}
                                {/* <p className="text-sm lg:text-base text-slate-200 leading-relaxed break-words whitespace-normal w-full">
                                    {modal.subtitle}
                                </p> */}
