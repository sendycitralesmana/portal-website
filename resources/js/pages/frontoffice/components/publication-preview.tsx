...
const PublicationPreview: React.FC<Props> = ({ publication }) => {
  ...
  const renderPublications = (data: PublicationItem[]) => (
    <div className="w-full grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
      {data.map((item) => (
        ...
        <Link key={item.id} href={`/publikasi/${activeTab}/${item.id}`}>
  ...
  return (
    <section className="w-full mt-14">
      <div className="container flex flex-col gap-5 items-center px-6 md:px-20">
        <h2 className="font-extrabold max-w-2xl text-center text-base sm:text-xl md:text-2xl text-[color:var(--primary-navy)] dark:text-white">PUBLIKASI</h2>
