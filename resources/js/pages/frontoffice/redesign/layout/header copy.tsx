...
const tugasFungsiMenuItems: MenuItem[] = [
  ...
  { label: "Tindak Pidana Tertentu", href: "/tugas-fungsi/tindak-pidana-tertentu" },
...
const publikasiMenuItems: MenuItem[] = [
  ...
  { label: "Media Sosial", href: "/publikasi/media-sosial" },
  ...
  { label: "Statistik", href: "/publikasi/statistik" },
]
...
export default function Header() {
  ...
  return (
    <>
      <header className="fixed top-0 left-0 right-0 z-50 border-b border-border bg-background/80 backdrop-blur-md">
        ...
        <div className="md:pl-16 lg:pl-24 mx-auto px-6">
          <div className="flex items-center justify-between h-20">
            ...
            {/* DESKTOP MENU */}
            <nav className="hidden md:flex gap-14 font-bold h-full items-center">
              {menus.map((menu) => (
                <button
                  key={menu}
            ...
            <div className="hidden md:flex items-center gap-4">
              <div className="relative">
                ...
                />
                <input className="bg-muted pl-9 pr-4 py-1.5 rounded-full text-sm border border-border focus:outline-none focus:ring-2 focus:ring-blue-500 transition" />
              </div>

              <div className="w-8 h-8 bg-blue-900 text-white rounded-full flex items-center justify-center text-xs font-medium">
                ID
              </div>
        ...
        {activeMenu && (
          <div className="absolute left-0 right-0 flex justify-center top-20 hidden md:flex">
            <div className="w-full max-w-7xl bg-background border border-border shadow-xl rounded-b-2xl overflow-hidden">
              ...
              <div className="bg-gradient-to-r from-blue-900 to-blue-700 text-white px-6 py-3 flex justify-between items-center">
                  ...
                  {getMenuItems().map((item, index) =>
                    item.external ? (
                      <a
                        ...
                        onClick={closeMenu}
                        className="block px-6 py-4 text-sm font-bold border-b border-border hover:bg-blue-50 dark:hover:bg-blue-900 transition"
                      >
                        {item.label}
                      </a>
                    ) : (
      ...
      {/* MOBILE MENU */}
      {mobileOpen && (
        <div className="md:hidden fixed top-20 left-0 right-0 bg-background border-t border-border shadow-lg z-40">
          ...
          <div className="px-4 py-4 border-b border-border bg-muted/40">
            <div className="flex items-center gap-3">
              ...
              <div className="relative flex-1">
                ...
                <input
                  placeholder="Cari..."
                  className="w-full bg-background pl-9 pr-3 py-2 rounded-full text-sm border border-border focus:outline-none focus:ring-2 focus:ring-blue-500 transition"
                />
              </div>

              {/* ID */}
              <div className="w-9 h-9 bg-blue-900 text-white rounded-full flex items-center justify-center text-xs font-medium shrink-0">
                ID
              </div>
