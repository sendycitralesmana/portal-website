...
const WorkUnitPagination = ({ workUnits }: { workUnits: WorkUnitType }) => {
  ...
  const { prev, next, pages } = createLimitedPagination(workUnits.links);

  return (
    <div className="flex flex-col items-center justify-between w-full py-2 lg:flex-row">
      <div>
        {workUnits.from} - {workUnits.to} of {workUnits.total}
      </div>
...
export default WorkUnitPagination;
