...
const AddSorotPage = () => {
  ...
  function handleFormSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    post(route('backoffice.sorot.store'));
  }
  ...
  function handleNewsChange(newsId: string) {
    ...
    fetch(route('backoffice.berita.show', newsId))
      .then((res) => {
  ...
  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      <Head title="Tambah Sorot" />
      <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
        <Card>
          <CardHeader>
            <CardTitle>Tambah Sorot</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="col-span-2 md:col-span-1">
                  ...
                  <ReactSelect
                    ...
                    options={news.map((n) => ({ value: n.id, label: n.title }))}
                    value={news
                      .map((n) => ({ value: n.id, label: n.title }))
                      .find((opt) => opt.value === data.news)}
                    onChange={(selected) => handleNewsChange(selected?.value || '')}
