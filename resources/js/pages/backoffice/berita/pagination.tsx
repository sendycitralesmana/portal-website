...
const BeritaPagination = ({ news }: { news: NewssType }) => {
  ...
  const { prev, next, pages } = createLimitedPagination(news.links);

  return (
    <div className="flex flex-col items-center justify-between w-full py-2 lg:flex-row">
      <div>
        {news.from} - {news.to} of {news.total}
      </div>
...
export default BeritaPagination;
