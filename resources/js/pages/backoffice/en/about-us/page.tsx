...
const breadcrumbs: BreadcrumbItem[] = [
  { title: 'About Us', href: '/backoffice/en/about-us' },
];
...
export default function AboutUsPage() {
  ...
  const handleDelete = (id: string) => {
    router.delete(`/backoffice/en/about-us/${id}/delete`);
  };

  return (
    <AppLayoutEn breadcrumbs={breadcrumbs}>
      <Head title="About Us">
        <meta name="description" content="Halaman About Us" />
        <link rel="icon" href="/images/favicon.ico" type="image/x-icon" />
      </Head>
      <div className="flex flex-col gap-4 p-4">
        <Card className="shadow-md">
        <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle>Data About Us</CardTitle>
            {!aboutUs && (
              <Button asChild>
                <Link href="/backoffice/en/about-us/add" className="flex items-center gap-1">
                  <Plus size={18} /> Add
                </Link>
              </Button>
            )}
            ...
            {aboutUs ? (
              <div className="flex flex-col sm:flex-row gap-6">
                ...
                <div className="flex-1 space-y-2 text-sm sm:text-base">
                  <p><strong>Address:</strong> {aboutUs.address}</p>
                  <p><strong>Phone:</strong> {aboutUs.phone}</p>
          ...
          {aboutUs && (
            <CardFooter className="flex justify-end gap-2">
              <Button asChild>
                <Link href={`/backoffice/en/about-us/${aboutUs.id}/edit`} className="flex items-center gap-1">
                  <PenBox size={18} /> Edit
                </Link>
              </Button>
              <AlertDialog>
