...
const RepresentativePagination = ({ representatives }: { representatives: RepresentativesType }) => {
  ...
  const { prev, next, pages } = createLimitedPagination(representatives.links);

  return (
    <div className="flex flex-col items-center justify-between w-full py-2 lg:flex-row">
      <div>
        {representatives.from} - {representatives.to} of {representatives.total}
      </div>
...
export default RepresentativePagination;
