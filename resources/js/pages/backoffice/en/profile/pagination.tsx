...
const ProfilePagination = ({ profiles }: { profiles: ProfileType }) => {
  ...
  const { prev, next, pages } = createLimitedPagination(profiles.links);

  return (
    <div className="flex flex-col items-center justify-between w-full py-2 lg:flex-row">
      <div>
        {profiles.from} - {profiles.to} of {profiles.total}
      </div>
