...
export default function PublikasiPage() {
  ...
  const handleSearch = useRef(
    debounce((query: string) => {
      router.get('/backoffice/publikasi', { search: query }, { preserveState: true, replace: true });
    }, 500)
  ).current;
  ...
  const handleDelete = (id: number) => {
    router.delete(`/backoffice/publikasi/${id}/delete`);
  };
    ...
    <AppLayout breadcrumbs={breadcrumbs}>
      ...
      <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
        <Card className='shadow-md'>
          <CardHeader>
            <CardTitle>Data Publikasi</CardTitle>
          </CardHeader>
          <CardContent className='space-y-8'>
            <div className='flex items-center justify-between'>
              ...
              <Button>
                <Link href='/backoffice/publikasi/add' className='flex items-center'>
              ...
              <TableBody>
                {publications.data?.map((app, index) => (
                  <TableRow key={app.id}>
                    ...
                    <TableCell>
                      {app.document_url && app.document_url.toLowerCase().endsWith('.pdf') ? (
                        <a href={`/backoffice/publikasi/${app.id}/preview-document`} target="_blank">
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem asChild>
                            <Link href={`/backoffice/publikasi/${app.id}/edit`} className='flex items-center'>
                              <PenBox className='ml-1 mr-1' /> Ubah
                            </Link>
                          </DropdownMenuItem>
                          <DropdownMenuItem asChild>
                            <Link href={`/backoffice/publikasi/${app.id}/detail`} className='flex items-center'>
          ...
          <CardFooter className="flex flex-col items-center justify-between w-full py-2 border-t lg:flex-row">
            <PublikasiPagination publications={publications} />
          </CardFooter>
        </Card>
      </div>
    </AppLayout>
