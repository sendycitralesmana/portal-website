...
const PublikasiPagination = ({ publications }: { publications: PublicationType }) => {
  ...
  const { prev, next, pages } = createLimitedPagination(publications.links);

  return (
    <div className="flex flex-col items-center justify-between w-full py-2 lg:flex-row">
      <div>
        {publications.from} - {publications.to} of {publications.total}
      </div>
...
export default PublikasiPagination;
