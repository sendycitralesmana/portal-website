...
import { ArrowLeft, RotateCcw, Save, FileText, Shield } from 'lucide-react';
import React from 'react';

interface SubjekTerlindung {
    id: number;
    kategori: string;
    judul: string;
    deskripsi: string;
...
interface EditSubjekTerlindungProps {
    subjekTerlindung: SubjekTerlindung;
}

export default function EditSubjekTerlindung({
    subjekTerlindung,
}: EditSubjekTerlindungProps) {
    const [isResetting, setIsResetting] = React.useState(false);
    const [newImagePreview, setNewImagePreview] = React.useState<string | null>(null);

    const { data, setData, reset, post, processing, errors } = useForm({
        kategori: subjekTerlindung.kategori ?? '',
        judul: subjekTerlindung.judul ?? '',
        deskripsi: subjekTerlindung.deskripsi ?? '',
                    ...
                    <CardContent className="p-6">
                        <form className="space-y-6" onSubmit={onHandleSubmit}>
                            ...
                            <div className="grid gap-1.5">
                                ...
                                {(subjekTerlindung.gambar || newImagePreview) && (
