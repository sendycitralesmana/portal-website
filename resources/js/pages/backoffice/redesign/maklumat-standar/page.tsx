...
import { FileText, Pencil, Plus, Search as SearchIcon, Briefcase } from 'lucide-react';

import DeleteDialog from './delete-dialog';
import Filter from './filter';
import MaklumatStandarPagination from './pagination';
import { PdfPreviewDialog } from '@/components/pdf-preview-dialog';

const MaklumatStandarPage = (props: any) => {
    const { data: maklumatStandars = [], meta } = props.maklumatStandars ?? {};

    console.log(maklumatStandars);
    ...
    useFilter({
        route: '/backoffice/maklumat-standar',
        values: params,
        only: ['maklumatStandars'],
        onLoading: (loading) => setIsLoading(loading),
    });

    return (
        <AppLayoutRedesign>
            <Head title="Maklumat dan Standar Pelayanan Publik">
                <meta name="description" content="Halaman Maklumat dan Standar Pelayanan Publik" />
            ...
            <div className="flex w-full flex-col">
                <div className="mb-8 flex flex-col items-start justify-between gap-y-4 lg:flex-row lg:items-center">
                    <HeaderTitle
                        title="Maklumat dan Standar Pelayanan Publik"
                        subtitle="Kelola data maklumat dan standar pelayanan publik yang ditampilkan pada halaman layanan publik."
                        icon={Briefcase}
                    />

                    <Button variant="blue" size="lg" asChild>
                        <Link href="/backoffice/maklumat-standar/create">
                ...
                <Card>
                            ...
                            <Table className="w-full table-auto">
                                ...
                                <TableBody>
                                    {isLoading ? (
                                        ...
                                        maklumatStandars.map((item: any, index: number) => (
                                                ...
                                                <TableCell className="align-top">
                                                    <div className="flex gap-2">
                                                        <Button className="bg-amber-400" variant="link" size="sm" asChild>
                                                            <Link href={`/backoffice/maklumat-standar/${item.id}/edit`}>
                            ...
                            {isLoading ? (
                                ...
                                maklumatStandars.map((item: any, index: number) => (
                                    <div key={item.id} className="rounded-xl border bg-white p-4 shadow-sm dark:bg-gray-900">
                                        ...
                                        {item.file && (
                                            <div className="mt-3">
                                                <ImagePreviewDialog src={item.file} alt={item.judul} />
                                            </div>
                                        )}
                    ...
                    <CardFooter className="flex w-full flex-col items-center justify-between border-t py-2 lg:flex-row">
                        <MaklumatStandarPagination meta={meta} />
                    </CardFooter>
                </Card>
            </div>
        </AppLayoutRedesign>
...
export default MaklumatStandarPage;
