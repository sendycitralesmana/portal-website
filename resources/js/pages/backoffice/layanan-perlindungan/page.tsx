...
const breadcrumbs: BreadcrumbItem[] = [
  { title: 'Layanan Perlindungan', href: '/backoffice/layanan-perlindungan' },
];
...
export default function LayananPerlindunganPage() {
  ...
  const handleSearch = useRef(
    debounce((query: string) => {
      router.get('/backoffice/layanan-perlindungan', { search: query }, { preserveState: true, replace: true });
    }, 500)
  ).current;
  ...
  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      <Head title="Layanan Perlindungan">
        <meta name="description" content="Halaman Layanan Perlindungan" />
      ...
      <div className="flex h-full flex-1 flex-col gap- rounded-xl p-4">
        <div className="flex justify-between items-center mb-4">
          {canAddMore && (
            <Button asChild>
              <Link href="/backoffice/layanan-perlindungan/add" className="flex items-center">
          ...
          {protectionService.map((item) => (
            <Card key={item.id} className="shadow-md rounded-lg p-4 mb-6 relative">
              ...
              <div className="absolute top-4 right-4">
                <Link href={`/backoffice/layanan-perlindungan/${item.id}/edit`}>
