...
const ApplicationPagination = ({ affiliates }: { affiliates: ApplicationType }) => {
  ...
  const { prev, next, pages } = createLimitedPagination(affiliates.links);

  return (
    <div className="flex flex-col items-center justify-between w-full py-2 lg:flex-row">
      <div>
        {affiliates.from} - {affiliates.to} of {affiliates.total}
      </div>
